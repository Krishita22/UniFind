...
// Step 2: confirm this user is the seller (marketplace_items.seller_id = users.id)
$checkStmt = $conn->prepare(
    'SELECT id FROM marketplace_items WHERE id = ? AND seller_id = ?'
);
...
// Step 3: delete it
$delStmt = $conn->prepare('DELETE FROM marketplace_items WHERE id = ?');
