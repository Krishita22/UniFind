...
$r = $conn->prepare("SELECT username, email, first_name FROM users WHERE id = ? LIMIT 1");
...
$stmt = $conn->prepare("UPDATE users SET has_warning = 0, warned_at = NULL WHERE id = ?");
$stmt->bind_param('i', $userId);
$ok = $stmt->execute();
$stmt->close();
if ($ok) {
    $del = $conn->prepare("DELETE FROM user_warnings WHERE user_id = ?");
    $del->bind_param('i', $userId);
    $del->execute();
    $del->close();
    $desc = "Warning revoked for @{$user['username']} ({$user['email']})";
    $log = $conn->prepare("INSERT INTO admin_activity_log (description, type) VALUES (?, 'user')");
    $log->bind_param('s', $desc);
    $log->execute();
    $log->close();
    $displayName = !empty($user['first_name']) ? $user['first_name'] : 'there';
    $subject = "Your UniFind Warning Has Been Revoked";
    $emailBody = '
<!DOCTYPE html>
<html lang="en">
<head>
...
<title>UniFind Warning Revoked</title>
</head>
          ...
          <tr>
            <td align="center" style="padding:40px 30px;">
              ...
              <p style="margin:0 0 20px 0; text-align:center; font-size:16px; line-height:1.6; color:#000000;">
                Good news! The warning on your UniFind account has been reviewed and revoked by the admin team.
              </p>
              <p style="color:#000000; font-size:15px; line-height:1.6; margin:20px 0 30px 0; text-align:center; padding:15px; background-color:#F9F7F4; border-left:4px solid #DD2635; border-radius:4px;">
                Your account is in good standing. Please continue to follow UniFind\'s community guidelines.
              </p>
